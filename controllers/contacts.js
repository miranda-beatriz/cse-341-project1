...
const createContact = async (req, res) => {
    ...
    const response = await mongodb.gettDatabase().db().collection('contacts').insertOne(contact);
...
const updateContact = async (req, res) => {
  ...
  const response = await mongodb.gettDatabase().db().collection('contacts').replaceOne({_id:contactId},contact);
...
const deleteContact = async (req, res) => {
      //#swagger.tags=['contacts']
    const contactId = new ObjectId(req.params.id);
    const response = await mongodb.gettDatabase().db().collection('contacts').deleteOne({_id:contactId});
